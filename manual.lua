...
elseif minetest.global_exists("doclib") then
	...
	doclib.create_manual("signs_bot", "RU", settings)
	local content = dofile(MP.."/manual_RU.lua")
	doclib.add_to_manual("signs_bot", "RU", content)
	...
	minetest.register_node("signs_bot:rumanual", {
		description = "Signs Bot Руководство (RU)",
		inventory_image = "doclib_book_inv.png",
		tiles = {
			-- up, down, right, left, back, front
			"doclib_book.png",
		...
		after_place_node = function(pos, placer, itemstack)
			minetest.get_meta(pos):set_string("infotext", "Signs Bot Руководство (RU)")
			minetest.get_meta(pos):set_string("formspec", doclib.formspec(pos, "signs_bot", "RU"))
		end,
		...
		on_receive_fields = function(pos, formname, fields, player)
			...
			minetest.get_meta(pos):set_string("formspec", doclib.formspec(pos, "signs_bot", "RU", fields))
		end,
